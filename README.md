...
v0.9.0 是当前公开 Beta，适合本地试用、从源码构建和参与开发。项目主页：
...
## v0.9 Public Beta 下载边界

- 源码可在满足上述要求的 Mac 上自行构建，SwiftPM 会为当前主机架构生成可执行文件。
- v0.9 prerelease 只附源码归档，不上传本地或 CI 生成的应用包。
- v0.9 的本地与 CI 打包产物目前只验证 Apple Silicon（arm64）；尚未提供经过验证的 Intel 或 Universal 2 应用包。
...
- v0.9 尚未提供经过 Developer ID 签名、公证和真实下载验收的预编译应用，也没有自动更新。
