...
v0.4.0 是项目的首个公开 Beta，适合本地试用、从源码构建和参与开发。项目主页：
...
## v0.4 Public Beta 下载边界

- 源码可在满足上述要求的 Mac 上自行构建，SwiftPM 会为当前主机架构生成可执行文件。
- v0.4 的本地应用包，以及 GitHub Release 中可能提供的预编译应用，目前都只验证 Apple Silicon（arm64）；尚未提供经过验证的 Intel 或 Universal 2 应用包。
...
![Markdown Lite 0.4 格式菜单、实时预览与远程图片隐私确认](docs/screenshots/markdown-lite-v04.png)

截图使用脱敏示例文档，展示原生编辑、格式菜单、增量语法着色、大纲、实时预览和远程图片默认不联网的确认状态。

## 已知边界

- v0.4 尚未提供经过 Developer ID 签名、公证和真实下载验收的预编译应用，也没有自动更新。
