...
感谢你愿意改进 [墨简（Mojian）](https://github.com/nemoob/mojian-editor)。请先阅读仓库级硬规则 [AGENTS.md](AGENTS.md)；本文只说明协作流程，避免重复维护编码规范。

## 开始之前

1. 搜索已有 [issue](https://github.com/nemoob/mojian-editor/issues) 和 [pull request](https://github.com/nemoob/mojian-editor/pulls)，确认问题尚未解决。
2. 对较大的行为变化先开 [issue](https://github.com/nemoob/mojian-editor/issues/new)，写清用户场景、数据安全影响和最小验收条件。
...
swift test --configuration release --disable-sandbox --no-parallel -Xswiftc -warnings-as-errors --filter WorkspaceEndToEndPerformanceTests
swift run --configuration release --disable-sandbox -Xswiftc -warnings-as-errors MarkdownLiteMac --self-check
RELEASE_BINARY_STRINGS="$(strings .build/release/MarkdownLiteMac)" && test -z "$(grep -E -- '--test-crash-recovery-(writer|reader)|--test-temp-root' <<<"$RELEASE_BINARY_STRINGS" || true)"
bash -n Scripts/package-app.sh
plutil -lint Support/Info.plist
```

新增行为必须补充标准 SwiftPM 测试；涉及性能或完整应用链路时，按测量范围扩展 release 端到端套件或 `--self-check`，并确保 `Scripts/check.sh` 实际执行该门禁。

涉及打包流程时，再执行：

```bash
bash Scripts/package-app.sh
plutil -lint dist/Mojian.app/Contents/Info.plist
codesign --verify --deep --strict dist/Mojian.app
```
...
1. 从 Finder 打开 `dist/Mojian.app`；“关于”版本必须等于 `Support/Info.plist`，主窗口可编辑。
2. 输入中文、标题、任务、代码块和表格，着色、大纲和预览须同步；`- 项目`、`9. 项目`、`- [x] 完成` 后按 Return 须分别生成 `- `、`10. `、`- [ ] `，空项退出列表，围栏内只换行。
3. 核对状态栏字符数和行数；选择包含中文、emoji 和组合字符的文本后须显示选区字符数，清空选区后须隐藏选区统计。
4. 打开超过 5 MiB 的临时 Markdown，须默认显示自动预览暂停说明；手动刷新一次可以生成预览，继续输入后须重新暂停。
5. 修改两个标签后退出重启；顺序、活动标签、已保存文件和未保存草稿须保持。
6. 用其他编辑器改写已打开的临时文件后再保存；必须阻止静默覆盖，并提供重载、另存为和明确覆盖入口。
7. 退出后仅破坏 `WorkspaceSession.json`；重启须提示“已从上一代会话恢复”。再退出并破坏两代；重启须显示持续警示，Finder 入口能定位证据，归档默认取消不改数据，确认后须显示精确目录并保留“恢复归档”入口。
8. 首实例有未保存文字时执行 `open -n dist/Mojian.app`；第二实例须提示“已在运行”并退出，首实例内容不变。
...
3. 执行 `bash Scripts/check.sh && bash Scripts/package-app.sh && codesign --verify --deep --strict dist/Mojian.app`，再完成上文 GUI smoke。
...
5. 创建并推送与 Info.plist 精确一致的 tag：`VERSION="$(plutil -extract CFBundleShortVersionString raw -o - Support/Info.plist)"; TAG="v${VERSION}"; test "$(git rev-parse HEAD)" = "$(git rev-parse origin/main)"; git tag -a "$TAG" -m "墨简（Mojian） $TAG"; git push origin "$TAG"`。
...
7. 不带资产路径地创建 prerelease：`TAG="v$(plutil -extract CFBundleShortVersionString raw -o - Support/Info.plist)"; gh release create "$TAG" --verify-tag --prerelease --generate-notes --title "墨简（Mojian） $TAG"`。
